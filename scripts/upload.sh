...
#
# Output: JSON with doc_id, filename, status (PENDING).
# Tip: pipe to jq to grab just the doc_id:
#   DOC_ID=$(./scripts/upload.sh paper.pdf | jq -r '.data.UploadDocument.doc_id')
set -eu
